...
from shared_schemas import BulkAssignmentRequest, CreateAssignmentRequest
...
async def student_assignments(request: Request, student_id: str):
    await require_service(request)
    svc, *_ = services(request)
    return await svc.list_student(student_id)


@router.get("/student/{student_id}/lessons/{lesson_id}/access")
async def student_lesson_access(request: Request, student_id: str, lesson_id: str):
    await require_service(request)
    _, _, assignments, progress, _ = services(request)
    rows = await assignments.list_for_lesson_student(lesson_id, student_id)
